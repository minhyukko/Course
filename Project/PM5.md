...
1. Each team member must submit a pull request to the repository. The PR must either fix an issue, refactor the code, add a new feature, or make some other substantial improvement to the code (i.e., not just adding a comment, but meaningful enhancements to the overall project documentation would be ok). The code in the PR may be generated by the student or by AI, adhering to the AI policy. In the PR, please indicate which option (AI or no-AI) was used for this activity by adding an appropriate label, comment, or updating the title.

2. Each team member must conduct a peer code review of a PR to your repository. Each team member should review a segment of code submitted by another teammate (AI- or human-generated, but not their own PR) and provide a review describing the following items:
